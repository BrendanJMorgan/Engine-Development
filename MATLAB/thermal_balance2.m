...
T_ref = zeros(1,length(x)); % K - reference temperature for combustion gas thermophysical properties
...
T_recovery = c_star_eff^2 .* T_free + Pr_gas.^(1/3) .* (Tc - T_free); % K - recovery temperature of combuston gas (fluid state if it slowed to velocity of body it is contacting, considering friction and heat transfer).
...
for i = 1:1:length(film_injection_x)
	...
	liquid_start = find(x >= film_injection_x(i)/dx, 1, 'first');
	liquid_end = find( cumtrapz(h_gas.*(T_recovery-T_sat_film).*dA) >= boiling_energy, 1, 'first');
...
for i = flow % Find heat and temperatures along engine contour
	...
	T_initial = [300,300]; % starting values
	fixed_T_cost_function = @(T_initial) T_cost_function(h_gas(i), h_cool(i), T_recovery(i), T_sat_film, T_ab(i), T_cool(i), x(i), r2(i)-r1(i), k_wall, mdot_film(1), film_injection_x(1), cp_film, T_injection, A_gas, A_cool(i), cooled(i));
	T_optimal = fminsearch(fixed_T_cost_function, T_initial);
...
function [T_wall_hot, T_wall_cold, T_film, q_gas] = compute_temperatures(h_gas, h_cool, T_recovery, T_sat_film, T_ab, T_cool, x, t_wall, k_wall, mdot_film, film_injection_x, cp_film, T_injection, A_gas, A_cool, cooled)

	if cooled == 1
		T_film = T_sat_film;
	elseif cooled == 0;
		T_film = 9999999999999;
	end
...
% 	T_film = T_film_prev + dT; % K
    ...
    q_gas = h_gas*A_gas*(T_ab-T_film); % W - Heat Flux per coolant channel, negative because it flows out of the gas
    T_wall_cold = q_gas ./ (h_cool*A_cool) + T_cool; % K - cold wall temperature, q_gas remains negative because it flows out of wall
    T_wall_hot = q_gas*t_wall/(k_wall*A_gas) + T_wall_cold - T_film; % K - temporary hot wall temperature DIFFERENCE for purposes of iteration, q_gas turned positive because it flows into wall
end

function cost = T_cost_function(h_gas, h_cool, T_recovery, T_sat_film, T_ab, T_cool, x, t_wall, k_wall, mdot_film, film_injection_x, cp_film, T_injection, A_gas, A_cool, cooled)
	[T_wall_hot, T_wall_cold, T_film, q_gas] = compute_temperatures(h_gas, h_cool, T_recovery, T_sat_film, T_ab, T_cool, x, t_wall, k_wall, mdot_film, film_injection_x, cp_film, T_injection, A_gas, A_cool, cooled);
	cost = T_film - T_wall_hot; % Film temperature and wall temperature should be the same
end

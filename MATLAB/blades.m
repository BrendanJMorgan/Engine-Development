...
blade_angle_inlet = atan2(u_inlet,v_inlet); % rad - inlet angle of blades relative to straight radial direction
...
blade_number = 1%2*round(0.5*(solidity * 2*pi*r_exit / blade_arc_length)); % number of blade, rounded to the nearest even number
...
function [blade_curve, control_points, control_error1, control_error2] = compute_blade_curve(control1, control2, sweep, radius1, radius2, polar1, polar2)
	...
	inlet_azimuth = atan2(inlet_point(2), inlet_point(1)) + polar1; % rad - blade inner edge absolute direction

...
r_inlet = r_eye; % m - radial location of blade inlets - typically vertially aligned with eye radius - this assumption can be changed
...
v_inlet = vdot_fuel/(2*pi*r_inlet*inlet_gap); % m/s - fluid velocity at the inlet, assumed to be entirely radial. Can be found with CFD instead for more accuracy (V_1 in pump handbook)
...
hydraulic_efficiency = 1 - 0.071 / vdot_fuel^0.25; % Jekat's Empirical Formula - wraps up all the various efficiencies into one formula, valid for all specific speeds

v_exit_tang = g*head_fuel / (hydraulic_efficiency*v_blade_exit);							% m/s - this is Euler's pump equation rearranged (V_theta,2 in pump handbook)                                                                                                                                                                                                                                                                                                                                                                                                                              
...
cost_function = @(params) cost_function_blade(params, r_inlet, r_exit, blade_angle_inlet, blade_angle_outlet, clock);
...
blade_thickness = 1/8*0.0254;%0.04*r_exit; % m

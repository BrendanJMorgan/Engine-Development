...
%% Number of Blades
% solidity = interp1([0.4, 3], [1.8, 0.9], specific_speed_fuel); % solidity is the optimal ratio of blade chord to blade spacing. Pump handbook page 2.36
% blade_number = solidity * 2*pi*r_exit / blade_arc_length;
...
function [blade_curve, control_points, control_error1, control_error2] = compute_blade_curve(control1, control2, sweep, radius1, radius2, polar1, polar2)
    ...
    s = [0; cumsum(sqrt(diff(blade_curve(:, 1)).^2 + diff(blade_curve(:, 2)).^2))]; % 1/m
    blade_curvature = abs(gradient(blade_curve(:, 1), s) .* gradient(gradient(blade_curve(:, 2), s), s) - gradient(blade_curve(:, 2), s) .* gradient(gradient(blade_curve(:, 1), s), s)) ./ (gradient(blade_curve(:, 1), s).^2 + gradient(blade_curve(:, 2), s).^2).^(3/2);
	max_blade_curvature = max(blade_curvature);
